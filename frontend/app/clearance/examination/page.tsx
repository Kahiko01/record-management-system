          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <BookOpen className="h-6 w-6 text-emerald-500" /> Examination Clearance
              </h1>
              <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Verify academic completion and approve clearance</p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 items-end">
              ...
              <div className="w-32">
                ...
                <select value={filters.level} onChange={(e) => setFilters({ ...filters, level: e.target.value })} className="w-full px-3 py-2.5 bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/50">
                  <option value="">All</option><option value="1">Level 1</option><option value="2">Level 2</option><option value="3">Level 3</option><option value="4">Level 4</option><option value="5">Level 5</option><option value="6">Level 6</option>
                </select>
              </div>
              ...
              <table className="w-full">
                <thead className="bg-gray-50 dark:bg-slate-800/50 border-b border-gray-200 dark:border-slate-800">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Exam Status</th>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-slate-800">
                  {viewMode === 'pending' ? (
                    requests.length === 0 ? (
                      <tr><td colSpan={5} className="px-6 py-16 text-center text-gray-500 dark:text-slate-500"><CheckCircle className="h-12 w-12 text-emerald-500/30 mx-auto mb-3" /><p className="font-medium">No pending examination clearances found!</p></td></tr>
                    ) : (
                      requests.map((request) => (
                        <tr key={request.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/30 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-2.5 py-1 text-xs font-bold rounded-full border ${getStatusColor(request.examination_clearance?.status || "pending")}`}>
                    ...
                    allStudents.length === 0 ? (
                      <tr><td colSpan={5} className="px-6 py-16 text-center text-gray-500 dark:text-slate-500">No students found matching your search.</td></tr>
                    ) : (
                      allStudents.map((student) => (
                        <tr key={student.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/30 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className="px-2.5 py-1 text-xs font-bold rounded-full border bg-gray-100 text-gray-600 dark:bg-slate-800 dark:text-slate-400 border-gray-200 dark:border-slate-700">❓ Check Details</span>
                          </td>
                          <td className="px-6 py-4">
                            <button onClick={() => alert(`View details for ${student.student_id}`)} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-bold transition-colors">View Details</button>
                          </td>
                        </tr>
                      ))
                    )
              ...
              <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl">
                ...
                {action === "approve" && (
                  <div className="mb-5 space-y-3">
                    <h3 className="text-sm font-bold text-gray-700 dark:text-slate-300 uppercase tracking-wider">Academic Verification Checklist</h3>
                    <label className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl hover:border-emerald-500/50 cursor-pointer transition-colors">
                      <input type="checkbox" checked={academicCheck.results_released} onChange={(e) => setAcademicCheck({ ...academicCheck, results_released: e.target.checked })} className="w-4 h-4 rounded bg-slate-800 border-slate-700 text-emerald-500 focus:ring-emerald-500/50" />
                      <span className="text-sm text-gray-700 dark:text-slate-300">Results Released</span>
                    </label>
                    <label className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl hover:border-emerald-500/50 cursor-pointer transition-colors">
                      <input type="checkbox" checked={academicCheck.program_completed} onChange={(e) => setAcademicCheck({ ...academicCheck, program_completed: e.target.checked })} className="w-4 h-4 rounded bg-slate-800 border-slate-700 text-emerald-500 focus:ring-emerald-500/50" />
                      <span className="text-sm text-gray-700 dark:text-slate-300">Program Completed</span>
                    </label>
                    <label className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl hover:border-emerald-500/50 cursor-pointer transition-colors">
                      <input type="checkbox" checked={academicCheck.graduation_approved} onChange={(e) => setAcademicCheck({ ...academicCheck, graduation_approved: e.target.checked })} className="w-4 h-4 rounded bg-slate-800 border-slate-700 text-emerald-500 focus:ring-emerald-500/50" />
                      <span className="text-sm text-gray-700 dark:text-slate-300">Graduation Approved</span>
                    </label>
                    <label className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl hover:border-emerald-500/50 cursor-pointer transition-colors">
                      <input type="checkbox" checked={academicCheck.no_missing_grades} onChange={(e) => setAcademicCheck({ ...academicCheck, no_missing_grades: e.target.checked })} className="w-4 h-4 rounded bg-slate-800 border-slate-700 text-emerald-500 focus:ring-emerald-500/50" />
                      <span className="text-sm text-gray-700 dark:text-slate-300">No Missing Grades</span>
                    </label>
                    <label className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl hover:border-emerald-500/50 cursor-pointer transition-colors">
                      <input type="checkbox" checked={academicCheck.credits_completed} onChange={(e) => setAcademicCheck({ ...academicCheck, credits_completed: e.target.checked })} className="w-4 h-4 rounded bg-slate-800 border-slate-700 text-emerald-500 focus:ring-emerald-500/50" />
                      <span className="text-sm text-gray-700 dark:text-slate-300">Required Credits Completed</span>
                    </label>
                  </div>
                )}
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => { setSelectedRequest(null); setAction(null); setRemarks(""); setAcademicCheck({ results_released: false, program_completed: false, graduation_approved: false, no_missing_grades: false, credits_completed: false }); }} className="flex-1 py-3 px-4 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-300 rounded-xl font-bold transition-colors">Cancel</button>
                </div>
              </div>
            </div>
          )}
